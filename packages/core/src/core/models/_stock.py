...
class StockSpotDB(Base):
    ...
    symbol = Column(String, primary_key=True)  # Stock symbol/ticker
    index = Column(String)  # no use, but can be used for company index or category
...
class StockHistoryDB(Base):
    ...
    symbol = Column(String, primary_key=True)  # 股票代码 (不带市场标识)
    adjust = Column(
...
class StockBusinessDB(Base):
    ...
    main_business = Column(String)  # 主营业务
    product_type = Column(String)  # 产品类型
    product_name = Column(String)  # 产品名称
    business_scope = Column(String)  # 经营范围
...
class StockBusinessCompositionDB(Base):
    ...
    category_type = Column(String, primary_key=True)  # 分类类型
...
class StockPledgeRatioDB(Base):
    ...
    symbol = Column(String, primary_key=True)  # 股票代码
    name = Column(String)  # 股票简称
    index = Column(String)  # 无用
    ...
    industry_code = Column(String)  # 所属行业代码
